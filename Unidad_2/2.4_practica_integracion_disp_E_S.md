...
3. Escriba el siguiente código dentro del archivo "main.c", cuyo objetivo es: 
	1. Emular un sistema de detección de incendio, que se muestra en la figura siguiente. Se considerará la entrada del detector de gas. Si el detector de gas está activado, entonces se activará la alarma. Si no, la alarma estará desactivada.
...
1. Modifique el hardware y el firmware para agregar un detector de alta temperatura (botón) que se activará cuando detecte que la temperatura es superior a las 50 °C.
...
2. Partiendo de lo anterior, modifique el hardware y el firmware para agregar un sensor de temperatura y humedad relativa (DHT11). 

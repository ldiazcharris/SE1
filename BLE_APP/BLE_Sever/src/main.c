...
void app_main()
{
    ...
    // Initializa el Non Volatile Storage (NVS) para guardar datos relevantes. 
    ret = nvs_flash_init();
    ...
    /**
     * Inicializa el Stack y el controlador Bluetooth por defecto
     * Host Controller Interface (HCI)
     * Link Layer (LL)
     * Physical Layer (PHY)
     * BT controller stack size, priority and HCI baud rate
     * 
     * 
    */
    ...
    /** Habilita el controlador en modo Bluetooth Low Energy (BLE)
     * 
     * Modos posibles:
    ...
    */
    ...
    // Se inicializan el controlador BT y el Bluedroid 
    ...
    /**
     * El Stack Bluetooth está activo y ejecutándose en este punto del flujo del programa; 
     * sin embargo, la funcionalidad de la aplicación aún no se ha definido. La funcionalidad 
     * se define reaccionando a eventos como lo que sucede cuando otro dispositivo intenta leer 
     * o escribir parámetros y establecer una conexión. Los dos principales administradores de 
     * eventos son los controladores de eventos GAP y GATT. La aplicación necesita registrar un 
     * CALLBACK para cada controlador de eventos para que la aplicación sepa qué funciones 
     * manejarán los eventos GAP y GATT.
     * 
     * En este ejemplo se muestran a continuación:
     */
...
/**
 * Se registran las aplicaciones que van a aestar disponibles en el servidor:
 * PROFILE_A_APP_ID
 * PROFILE_B_APP_ID
 */
    ...
    /**
     * Define la unidad máxima de transferencia máxima o unidad de transmisión máxima local.
     */
    esp_err_t local_mtu_ret = esp_ble_gatt_set_local_mtu(512); //máximo 517 mínimo 23
    if (local_mtu_ret){
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            ESP_LOGI("Recibi: ", "%s", param->write.value);
            ...
            if (gl_profile_tab[PROFILE_A_APP_ID].descr_handle == param->write.handle && param->write.len == 2)
            {
                ...
                if (descr_value == 0x0001)
                {
                    if (a_property & ESP_GATT_CHAR_PROP_BIT_NOTIFY)
                    {
                        ...
                        uint8_t notify_data[15] = "Hola ESP";
                        /*
